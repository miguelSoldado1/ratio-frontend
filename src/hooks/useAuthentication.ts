...
const clearAuthStorage = () => {
  for (const key of Object.keys(LOCAL_STORAGE_KEYS)) {
    localStorage.removeItem(LOCAL_STORAGE_KEYS[key as keyof typeof LOCAL_STORAGE_KEYS]);
  }
};
...
const getExpiresIn = (expiresIn: string) => {
  const current = new Date().getTime();
  return new Date(current + (Number(expiresIn) - SECONDS_BEFORE_REFRESH_TOKEN) * 1000).getTime().toString();
};
